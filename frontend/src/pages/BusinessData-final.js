...
const BusinessData = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      const res = await fetch(`${API_BASE_URL}/api/business-data/upload`, {
        method: 'POST',
        body: form,
      });
  ...
  const handleDownloadSample = () => {
    // open in a new tab so browser handles the file
    window.open(`${API_BASE_URL}/api/business-data/sample`, '_blank', 'noopener,noreferrer');
  };
  ...
  const handleTrainAI = async (dataId) => {
    try {
      ...
      // Optionally refresh a "status" column if your API returns one
      await fetchDatasets();
    } catch (e) {
        ...
        {visible.length === 0 && !loading ? (
          ...
          visible.map((d, idx) => (
            <React.Fragment key={d.id || d._id || idx}>
              ...
              >
                <ListItemText
                  primary={d.name || `Dataset ${idx + 1}`}
                  secondary={d.description || d.filename || ''}
                />
